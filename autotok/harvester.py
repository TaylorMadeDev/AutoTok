...
def harvest_video(
    ...
    try:
        ...
        segments = split_video(downloaded, destination, segment_seconds, progress)
        if progress:
...
def harvest_urls(
    ...
    for index, url in enumerate(clean_urls, start=1):
        ...
        results.append(harvest_video(url, category, segment_seconds, item_progress))
    return results
...
def harvest_playlist(
    ...
    return harvest_urls(urls, category, segment_seconds, progress)
